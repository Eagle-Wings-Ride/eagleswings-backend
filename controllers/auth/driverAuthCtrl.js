...
const registerDriver = async (req, res) => {
  const { fullname, email, password, phone_number, residential_address } =
    req.body;
  ...
  try {
    ...
    res
      ...
      .json({
        message:
          "Driver registered successfully. Please check your email to verify your account.",
      });
  } catch (error) {
...
const requestPasswordReset = async (req, res) => {
  ...
  try {
    ...
    driver.passwordResetOTP = req.otp;
    driver.passwordResetExpiry = req.otpExpiry;
    driver.passwordResetVerified = false;
    await driver.save();
...
const verifyPasswordResetOTP = async (req, res) => {
  ...
  try {
    const driver = await Driver.findOne({ email });
    if (!driver) return res.status(404).json({ message: "Driver not found" });
    if (!driver.passwordResetOTP || !driver.passwordResetExpiry)
      return res.status(400).json({ message: "No OTP request found" });
    if (
      driver.passwordResetOTP !== otp ||
      driver.passwordResetExpiry < new Date()
    )
      return res.status(400).json({ message: "Invalid or expired OTP" });

    driver.passwordResetVerified = true;
    await driver.save();
...
const resetPassword = async (req, res) => {
  ...
  try {
    const driver = await Driver.findOne({ email });
    if (!driver) return res.status(404).json({ message: "Driver not found" });
    if (!driver.passwordResetVerified)
      return res.status(403).json({
        message: "OTP verification required before resetting password",
      });

    const isSamePassword = await compare(newPassword, driver.password);
    if (isSamePassword)
      return res
        .status(400)
    ...
    driver.passwordResetVerified = undefined;
...
// Change passwords: for logged-in drivers who want to change their password
